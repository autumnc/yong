...
static void write_xism_status_file(CONNECT_ID *id);
...
int y_xim_init(const char *name)
{
	...
	int ret = xim.init();
	if (ret == 0) {
		/* Write initial status file after XIM init */
		CONNECT_ID *id = y_xim_get_connect();
		write_xism_status_file(id);
	}
	return ret;
}
	...
	if(s)
	{
		...
		else if(s[0]=='$')
		{
			...
			else if(!strcmp(s,"$REDIRECT()"))
			{
				...
				if(ret==-1)
				{
					y_ui_show_tip(YT("��ǰ���ò�֧����������"));
				}
				else if(ret==0)
				{
					y_ui_show_tip(YT("û��ƥ�������"));
				}
			}
			else
			{
...
static const char *xim_im_names[] = {
	"\xd3\xc0\xc2\xeb",   /* 永码 */
	"\xce\xe5\xb1\xca",   /* 五笔 */
	"\xc1\xbd\xb7\xd6",   /* 两分 */
	"\xb6\xfe\xb1\xca",   /* 二笔 */
...
static int xim_im_names_count(void)
{
	return sizeof(xim_im_names) / sizeof(xim_im_names[0]);
}
...
static void write_xism_status_file(CONNECT_ID *id)
{
	...
	if (id && id->state) {
		if (id->lang == LANG_CN) {
			int idx = im.Index;
			if (idx >= 0 && idx < xim_im_names_count())
				fprintf(fp, "%s", xim_im_names[idx]);
			else
...
void y_im_about_self(void)
{
	...
	pos+=sprintf(temp+pos,"%s%s\n",YT("����ʱ�䣺"),YONG_VERSION);
	/*pos+=*/sprintf(temp+pos,"%s%s",YT("��л��"),"stb, nanosvg");
...
int y_im_get_keymap(char *name,int len)
{
	...
	if(!ps)
	{
		...
		{
			char keymap[64];
			l_gb_to_utf8(YT("�ָ�ͼ"),keymap,sizeof(keymap));
